...
//	Fully commented as it never gets activated and error on gles
...
//	Lux: Anisotropic Lighting
	...
	//	Orthonormalize tangentDir to take normal into account
		worldTangentDir = normalize(worldTangentDir - dot(worldTangentDir, normalWorld) * normalWorld);
		float3 worldBitangent = cross(normalWorld, worldTangentDir);

	//	Old version:
	// 	Adjust worldTangentDir to per pix
	 	//float3 worldBitangent = cross(normalWorld, worldTangentDir);
	...
	//	Direct lighting
		...
		//res.rgb = worldTangentDir * 0.5 + 0.5;
