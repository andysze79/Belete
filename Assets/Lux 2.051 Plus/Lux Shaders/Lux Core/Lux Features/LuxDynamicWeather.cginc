...
//  Additional Inputs ------------------------------------------------------------------
    ...
    half4           _Lux_WorldMappedSnowTiling;
    half2           _Lux_WorldMappedSnowStrength;
#endif
...
//  Based on the work of Sébastien Lagarde: http://seblagarde.wordpress.com/2013/04/14/water-drop-3b-physically-based-wet-surfaces/
...
#if defined (_WETNESS_RIPPLES) || defined (_WETNESS_FULL)
    //  DEPRECATED Samples and returns animated ripple normals / Called by: AddWaterRipples 
    ...
    //  DEPRECATED: Add animated Ripples to areas where the Water Accumulation is high enough
...
/*  inline half3 AddWaterRipples(LuxFragment lux, float2 dx, float2 dy)
    {
        ...
        float2 rippleOffset = lerp(lux.offsetInWS, 0, lux.waterAmount.y * lux.waterAmount.y);

        half2 Ripple1 = ComputeRipple( (lux.worldPos.xz + rippleOffset) * _Lux_RippleTiling , dx, dy, animSpeed, Weights.x);
        half2 Ripple2 = ComputeRipple( (lux.worldPos.xz + rippleOffset + float2(0.55f, 0.37f)) * _Lux_RippleTiling , dx, dy, animSpeed * 0.85f, Weights.y); 
...
*/
    ...
    //  NEW: Add animated Ripples to areas where the Water Accumulation is high enough
    //  Returns the tweaked and adjusted Ripple Normal
    inline half3 AddWaterRipplesFromRenderTex(LuxFragment lux, float2 dx, float2 dy)
    {
...
// 2018
// 
        baseSnowAmount *= lux.uniqueSnowMaskValue;        

    //  Sample the Mask Texture with different tiling factors
        float2 snowMaskUVs = 0;
        float2 detailSnowMaskUVs = 0;
        if (_SnowMapping == 0) { 
            snowMaskUVs       = lux.extrudedUV.xy * _SnowMaskTiling;
            detailSnowMaskUVs = lux.extrudedUV.xy * _SnowDetailTiling;
        }
        if (_SnowMapping == 1) {
            lux.baseSnowUVs   = lux.worldPos.xz + lerp(lux.offsetInWS.xy, 0, lux.snowAmount.x * lux.snowAmount.x * 0.25);
            snowMaskUVs       = lux.baseSnowUVs * _Lux_WorldMappedSnowTiling.z;
            detailSnowMaskUVs = lux.baseSnowUVs * _Lux_WorldMappedSnowTiling.w;
        }
        fixed4 snowMask = tex2D(_Lux_SnowMask, snowMaskUVs );
        fixed4 detailSnowMask = tex2D (_Lux_SnowMask, detailSnowMaskUVs);
    ...
    //  Add custom snowmask
// This looks odd when using vertex colors as mask
//        snowAmount *= lux.uniqueSnowMaskValue;
    //  Sharpen snow (due to slope damp and unique mask)
        ...
        half2 finalSnowMask = snowMask.bb * detailSnowMask.bb;
    ...
    //  We have to calculate 2 snow values: one for the actual snow (x) and one for the melted one (y)
        ...
        snowAmount = finalSnowMask * snowAmount * lerp(detailSnowMask.bb, 1.0, baseSnowAmount).xx; // ps4 fix added: ".xx"
        return snowAmount;
    }
...
void Lux_DynamicWeather ( inout LuxFragment lux)
{
        ...
        lux.offsetInWS = lux.offset;

        #if !defined(LUX_STANDARD_CORE_INCLUDED)
            lux.offsetInWS = lux.offsetInWS_Surface * _UVRatio.z;
        #else
            half3 t = lux.tangentToWorld[0].xyz;
            half3 b = lux.tangentToWorld[1].xyz;
            half3 n = lux.tangentToWorld[2].xyz;
            half3x3 tbn = half3x3(t, b, n);
            float2 objectScale_TextureScale_Ratio = lux.scale / _MainTex_ST.xy; // <- first one might not be precise enough? 
            lux.offsetInWS = mul( float3(lux.offset.xy * objectScale_TextureScale_Ratio , 0), tbn).xz * _UVRatio.z;
        //  height of watersurface
            lux.offsetWaterInWS = mul( float3(lux.offsetWaterInWS * objectScale_TextureScale_Ratio , 0), tbn).xz * _UVRatio.z;
        #endif
            ...
            // No branching when using tessellation as otherwise it breaks
                ...
                //  Compute ripple normal
                        ...
                        if (_Lux_RainfallRainSnowIntensity.y > 0.0) {
                            ...
                            // NEW:
                            rippleNormal = AddWaterRipplesFromRenderTex (lux, dx, dy);
                        }
                    #endif
                //  Compute flow normal
                    #if defined (_WETNESS_FLOW) || defined (_WETNESS_FULL)
...
//  Snow
        ...
        suppressionFactor = lerp(suppressionFactor, 1.0 - lux.snowAmount.x * _SnowOpacity, lux.snowAmount.x);
            ...
            #if defined (_PARALLAXMAP) || defined (EFFECT_BUMP)
			//	Snow local mapping
                float4 i_tex_snow = (_SnowMapping == 0) ?
                lerp(lux.extrudedUV.xyxy, lux.extrudedUV.xyxy - lux.offset.xyxy / float4(_SnowTiling, _SnowDetailTiling), lux.snowAmount.x * 0.5) * float4(_SnowTiling, _SnowDetailTiling)
                :
			//	Snow world mapping
                (lux.worldPos.xzxz + lerp(lux.offsetInWS.xyxy, 0, lux.snowAmount.x * 0.5) ) * _Lux_WorldMappedSnowTiling.xxyy;
            #else
				float4 i_tex_snow = (_SnowMapping == 0) ?
			//	Snow local mapping
				lux.extrudedUV.xyxy * float4(_SnowTiling, _SnowDetailTiling)
				:
			//	Snow world mapping
				lux.worldPos.xzxz * _Lux_WorldMappedSnowTiling.xxyy;
            #endif
			...
			// dx dy!!!!!!! 
			float2 i_tex_snow_dx = i_tex_snow;

            float4 dx = ddx( i_tex_snow);
            float4 dy = ddy( i_tex_snow);
            
			// Smooth the normalBlendFactor by taking the outer bounds or smooth snowmask into account
            lux.normalBlendFactor = (lux.snowAmount.x * lux.snowAmount.y);

            half4 combinedNormalSmoothness = tex2Dgrad(_Lux_SnowWaterBump, i_tex_snow.xy , dx.xy, dy.xy );

			half2 finalstrength = lerp( half2(_SnowNormalStrength, _SnowDetailStrength), _Lux_WorldMappedSnowStrength.xy, _SnowMapping );
			lux.snowNormal = Lux_UnpackScaleNormal (combinedNormalSmoothness.ag, lux.normalBlendFactor * saturate(_Lux_SnowAmount * _Lux_WaterToSnow * _SnowAccumulation.y + _SnowAccumulation.x) * finalstrength.x );

		//	Normal is not in TangetSpace if sampled in WS!
            UNITY_BRANCH
            if (lux.detailBlendState > 0.001 ) {
                half3 snowdetailNormal = Lux_UnpackScaleNormal (  tex2Dgrad(_Lux_SnowMask, i_tex_snow.zw, dx.zw, dy.zw).ag, finalstrength.y * lux.detailBlendState);
                lux.snowNormal = Lux_BlendNormals(lux.snowNormal, snowdetailNormal );   
            }

        //  Combine or not combine normals depending on snow mapping mode!
            lux.tangentNormal = (_SnowMapping == 0) ? lerp(lux.tangentNormal, lux.snowNormal, saturate(lux.normalBlendFactor) ) : lux.tangentNormal;
...
//  NOTE: SnowWorldNormal has to be brought to WorldSpace
        ...
        #define LUX_APPLY_DYNAMICWEATHER \
            ...
            o.SnowWorldNormal.xyz = mul( lux.snowNormal.xyz, SnowTangentToWorld); \
            o.SnowWorldNormal.w = lux.normalBlendFactor * _SnowMapping; \
            half3 diffuseScatter = 0; \
            half NdotV = dot(lux.snowNormal, lux.eyeVecTangent); \
            NdotV *= NdotV; \
...
// half NdotV = dot(o.SnowWorldNormal, lux.eyeVec); \