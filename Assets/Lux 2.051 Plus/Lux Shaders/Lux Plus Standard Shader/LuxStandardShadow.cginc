...
#if defined(_ALPHABLEND_ON) || defined(_ALPHAPREMULTIPLY_ON) || defined(LOD_FADE_CROSSFADE)
...
#if defined(_ALPHATEST_ON) || defined(_ALPHABLEND_ON) || defined(_ALPHAPREMULTIPLY_ON) || defined(LOD_FADE_CROSSFADE)
    ...
    )
{
    UNITY_SETUP_INSTANCE_ID(v);
        ...
        UNITY_TRANSFER_INSTANCE_ID(v, os);
    #endif
    ...
    ) : SV_Target
{
    ...
    #if defined(LOD_FADE_CROSSFADE) && defined(UNITY_STANDARD_USE_DITHER_MASK)
        // + 0.05 to slightly overlap shadows?!
        //float fade = unity_LODFade.x + ((unity_LODFade.x > 0.5) ? 0.1 : 0);
        //half clipRef = tex3D(_DitherMaskLOD, float3(vpos.xy*0.25f, saturate(fade) * 0.9375f)).a;
        //clip (clipRef - 0.01h);
    //  2019
