...
//-------------------------------------------------------------------------------------
	...
	void LuxApplyDitherCrossFade(float2 vpos) {
		//vpos /= 4;
		// Old code
			//vpos.y = frac(vpos.y) * 0.0625 + unity_LODFade.y;
...
//	In case we sample smoothness from Albedo alpha we simply overwrite it here. Should be optimized by the compiler.
	#if defined (_SMOOTHNESS_TEXTURE_ALBEDO_CHANNEL_A) && !defined(_ALPHATEST_ON)
		o.oneMinusRoughness = lux.albedoAlpha.a;
	#endif

	o.oneMinusRoughness *= _GlossMapScale;
...
//	Lux: We always need world position for e.g. area lights
	...
	o.posWorld.w = o.pos.w; //o.posWorld.w = distance(_WorldSpaceCameraPos, posWorld);
//	Lux
	...
	) : SV_Target
{
	...
	#if defined(LOD_FADE_CROSSFADE) && !defined(SHADER_API_D3D9)
        LuxApplyDitherCrossFade(i.pos.xy);
    #endif
...
//	half4 c = UNITY_BRDF_PBS(s.diffColor, s.specColor, s.oneMinusReflectivity, s.oneMinusRoughness, s.normalWorld, viewDir, light, noIndirect);
